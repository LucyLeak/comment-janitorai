...
export default async function handler(req, res) {
  ...
  comments.forEach(comment => {
    ...
    if (comment.pinned) {
      ...
      // Render label above
      renderedLines.push(
        `<text x="${padding}" y="${labelYOffset}" class="pinned-label">
          <tspan>📌 pinned</tspan>
        </text>`
      );
      yOffset += lineHeight; // Move yOffset for the comment itself
    }
    ...
    if (comment.liked_by_owner) {
      renderedLines.push(`<image 
        ...
        "
        x="${width - padding - 10}" y="${comment.pinned ? labelYOffset - 10 : yOffset - 10}" width="20" height="20" clip-path="circle(20)"/>
      `);
    }
